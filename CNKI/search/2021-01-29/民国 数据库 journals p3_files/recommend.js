var ipUrl = "oversea-1cnki-1net-10097d65n0090.erf.sbb.spk-berlin.de";//此处的ip是Krs发布机器的ip地址  ，外网发布时则修改为域名。
var indexArray = ["http://www-1cnki-1net-10097d65n0090.erf.sbb.spk-berlin.de"];
var ISIE;
...
//func 回调函数
function WriteActionLog(userName, cnkiUserKey, curUrl, referUrl,func) {
    if (curUrl == "") {
        return;
    }
    ...
    var serverUrl = "//" + ipUrl + "/KRS/KRSWriteHandler.ashx";
    ...
    CoreDomainClass.Load(geturl, function () { if (typeof func === "function") func(); });
}
...
function WriteClickLog() {
    ...
    parent.$("#recommendContent").on("click", "a", function () {
        var KeyWord = $(this).attr("title");
        var Href = $(this).attr("href");
        var type = "recommend";
        var serverUrl = "//" + ipUrl + "/KRS/ClickHandler.ashx";
